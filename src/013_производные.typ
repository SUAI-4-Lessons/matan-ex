...
Фактически это продолжение хода по касательной на опр. значение $delta x$, так как если мы опишем это с использование геометрического смысла производной, то $ d y = tg alpha dot Delta x $
